...
public class RacingBalloonEvent extends AbstractTimedEvent {
    ...
    public void init() {
        Entropy.getInstance().eventHandler.getActivePlayers().forEach(player -> {
            ...
            HolderSet.Named<Item> allHarnesses = BuiltInRegistries.ITEM.getOrThrow(ItemTags.HARNESSES);
            Item harness = allHarnesses.getRandomElement(player.level().getRandom()).map(Holder::value).orElse(Items.WHITE_HARNESS);
