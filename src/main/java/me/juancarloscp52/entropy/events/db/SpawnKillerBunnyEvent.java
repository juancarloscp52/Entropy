    ...
    public void init() {
        Entropy.getInstance().eventHandler.getActivePlayers().forEach(player -> {
            ...
            for(int i = 0; i < bunnyAmount; i++) {
                EntityType.RABBIT.spawn(player.getServerWorld(), rabbit -> rabbit.setVariant(RabbitType.EVIL), player.getBlockPos().add(random.nextBetween(-4, 4), random.nextInt(2), random.nextBetween(-4, 4)), SpawnReason.SPAWN_EGG, false, false);
            }
        });
    }
}
