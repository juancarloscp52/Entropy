    ...
    public void init() {
        Entropy.getInstance().eventHandler.getActivePlayers().forEach(player -> {
            ...
            for(int i = 0; i < bunnyAmount; i++) {
                EntityType.RABBIT.spawn(player.serverLevel(), rabbit -> rabbit.setVariant(Variant.EVIL), player.blockPosition().offset(random.nextIntBetweenInclusive(-4, 4), random.nextInt(2), random.nextIntBetweenInclusive(-4, 4)), MobSpawnType.SPAWN_EGG, false, false);
            }
        });
    }
}
