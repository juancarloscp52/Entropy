...
public abstract class AbstractTimedEvent implements Event{
    ...
    public void renderQueueItem(MatrixStack matrixStack, float tickdelta, int x, int y) {
        ...
        DrawableHelper.drawTextWithShadow(matrixStack,client.textRenderer, new TranslatableText(translationKey),client.getWindow().getScaledWidth()-size-40,y, MathHelper.packRgb(255,255,255));
        if(!this.hasEnded()){
